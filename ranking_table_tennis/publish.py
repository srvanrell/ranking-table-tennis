...
def main(last=True, tournament_num=None, config_initial_date="220101"):
    """Publish the results to a new sheet and markdown files.
    ...
    Only local files will be created.
    ...
    helpers.publish_rating_sheet(tournaments, rankings, players, tid, prev_tid)
    helpers.publish_rating_sheet(tournaments, rankings, players, tid, prev_tid, all_players=True)

    # Publish points assigned in each match
    helpers.publish_rating_details_sheet(tournaments, rankings, players, tid, prev_tid)

    # Publish matches as they were loaded
    helpers.publish_matches_sheet(tournaments, rankings, players, tid)

    # Publish formated masters championship of selected tournament
    helpers.publish_championship_sheets(tournaments, rankings, players, tid, prev_tid)

    # Publish points assigned per best round reached
    helpers.publish_championship_details_sheet(tournaments, rankings, players, tid, prev_tid)

    # Saving complete histories of players
    helpers.publish_histories_sheet(tournaments, rankings, players, tid, prev_tid)

    # Publshing initial_ranking
    helpers.publish_initial_rating_sheet(tournaments, rankings, players, tid)

    # Publish statistics
    helpers.publish_statistics_sheet(tournaments, rankings, players, tid, prev_tid)
